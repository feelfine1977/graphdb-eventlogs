...
file_to_open = os.path.expanduser("~/neo4j/Data/ECESample.csv")
file_to_save = os.path.expanduser("~/neo4j/Data/ECEsamplePerformance.csv")
...
if(step_Sample):
    ...
    #'ECEsample.csv' 
    perfFileName = file_to_save
    #'ECEsamplePerformance.csv'
else:
    fileName = 'ECEfull.csv'
    perfFileName = 'ECEfullPerformance.csv'
    
# data model specific to ECE
...
model_entities = [['POItem','cID', 'WHERE EXISTS(e.cID)'], # Purchase Order Items (Original Case ID)
                 ...
                 #['Resource','resource', 'WHERE EXISTS(e.resource) AND e.resource =  "A" AND e.resource <> NULL'
                 # ], # resources/users
                  #['Vendor','cVendor','WHERE EXISTS(e.cVendor)']
                  ] # vendors
...
model_relations = [['PO','PO','POItem','cPOID']]
...
def CreateEventQuery(logHeader, fileName, LogID = ""):
    query = f'USING PERIODIC COMMIT LOAD CSV WITH HEADERS FROM \"{fileName}\" as line'
...
if step_LoadEventsFromCSV:
    ...
    qCreateEvents = CreateEventQuery(header, fileName, 'ECE') #generate query to create all events with all log columns as properties
